...
> An open-source, multi-model autonomous trading platform.
...
3. **Set up environment variables**
   ...
   ACTIVE_MODEL="deepseek" # Options: deepseek, openai, gemini, local
...
While the default setup runs a single bot instance based on `ACTIVE_MODEL`, the architecture supports running multiple bots. You can achieve this by:

- Setting up multiple cron jobs calling the run endpoint with different parameters (requires code customization to accept params).
- Or simply deploying multiple instances of the application with different `.env` configurations (e.g., one for BTC with GPT-4, one for ETH with DeepSeek).
